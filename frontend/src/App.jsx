  ...
  return (
    <main className="page">
      ...
      <section className="card form-grid">
        ...
        <button onClick={compare}>Compare Pricing</button>
      </section>
      ...
      <section className="card">
        ...
        <div className="table-wrap"><table><thead><tr><th>Provider</th><th>Service</th><th>Instance</th><th>vCPU</th><th>Memory</th><th>Hourly</th><th>Monthly</th><th>Match</th></tr></thead><tbody>
          {results.length === 0 ? <tr><td colSpan="8">Run a comparison to see results.</td></tr> : results.map((r,i)=><tr key={i}><td>{r.provider}</td><td>{r.service}</td><td>{r.instance_type}</td><td>{r.vcpu}</td><td>{r.memory_gb} GB</td><td>${r.hourly_price}</td><td>${r.monthly_price}</td><td>{r.match_quality}</td></tr>)}
        </tbody></table></div>
      </section>
    </main>
  );
